...
impl AudioInfo {
    ...
    fn process_audio_file(entry: &walkdir::DirEntry) -> Result<AudioFile, ProcessError> {
        if entry.file_type().is_file() && entry.path().extension() == Some("flac".as_ref()) {
            let (samples, stream_info) = Self::load_flac(entry.path())?;

            let total_samples = stream_info.samples.ok_or(ProcessError::NoSamplesFound)?;

            let crc32_checksum = Self::generate_crc32(samples);
    ...
    fn generate_crc32(samples: Vec<i16>) -> String {
    ...
    fn load_flac(path: &Path) -> Result<(Vec<i16>, StreamInfo), FlacLoadError> {
        let mut flac_file = File::open(path)?;
        let mut flac_data = Vec::new();
        flac_file.read_to_end(&mut flac_data)?;

        // Decode FLAC data to PCM samples
        let mut reader = claxon::FlacReader::new(&flac_data[..])?;
        let samples: Vec<i16> = reader
            .samples()
            .map(|sample| sample.unwrap_or(0) as i16)
            .collect();

        Ok((samples, reader.streaminfo()))
    }
}
